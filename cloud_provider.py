...
def export_resource_to_cloud_storage(
    ...
    **kwargs: P.kwargs,
    ...
    result_filename = rq_job_meta.result_filename
    if storage.prefix:
        normalized_prefix = storage.prefix.rstrip("/")
        if not result_filename.startswith(f"{normalized_prefix}/"):
            result_filename = f"{normalized_prefix}/{result_filename.lstrip('/')}"
    storage.upload_file(Path(file_path), result_filename)
